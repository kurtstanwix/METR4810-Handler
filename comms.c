 ...
 * Author: kurts_000
 *
 * Created on 22 June 2020, 1:05 PM
 */
...
/**
 * Sets the speed of the selected Motor based on a received command
 * @param jointNum joint number of motor to set speed of
 * @param speed speed to set the motor
 */
void set_speed(int16_t jointNum, int16_t speed) {
    HARDWARE_PWM_JOINT_1_STOP();
    HARDWARE_PWM_JOINT_3_STOP();
    
    if (speed > 0) {
        HARDWARE_PWM_DUTY_CYCLE_SET(JOINT_1_3_PWM_1, 1000 - (uint16_t) speed);
        HARDWARE_PWM_DUTY_CYCLE_SET(JOINT_1_3_PWM_2, 1000);
        if (jointNum == 1) {
            HARDWARE_PWM_JOINT_1_START();
        } else if (jointNum == 3) {
            HARDWARE_PWM_JOINT_3_START();
        }
    } else if (speed < 0) {
        HARDWARE_PWM_DUTY_CYCLE_SET(JOINT_1_3_PWM_1, 1000);
        HARDWARE_PWM_DUTY_CYCLE_SET(JOINT_1_3_PWM_2, 1000 - (uint16_t) (speed * -1));
        if (jointNum == 1) {
            HARDWARE_PWM_JOINT_1_START();
        } else if (jointNum == 3) {
            HARDWARE_PWM_JOINT_3_START();
...
void process_packet(BUFFER_OBJ *packet) {
    if (compare_strings(packet->buffer, commands[COMMAND_DEG_INDEX], COMMAND_LENGTH)) {
        packet->tail -= 3;
        // Get the second parameter
        int16_t deg = get_param(packet, ',');
        if (deg == INVALID_PARAM) {
            return; // Invalid packet
        }
        packet->tail--; // move tail to end of next parameter
        // Get the first parameter
        int16_t jointNum = get_param(packet, commands[COMMAND_DEG_INDEX][COMMAND_LENGTH - 1]);
        if (jointNum == INVALID_PARAM) {
            return; // Invalid packet
        }
        if (deg >= 1000 && deg <= 2000) {
            switch (jointNum) {
                case 2:
                    HARDWARE_PWM_PULSE_WIDTH_SET(JOINT_2_PWM, deg);
                    break;
                case 4:
                    HARDWARE_PWM_PULSE_WIDTH_SET(JOINT_4_PWM, deg);
                    break;
                case 5:
                    HARDWARE_PWM_PULSE_WIDTH_SET(JOINT_5_PWM, deg);
                    break;
            }
        }
        ///////////////////////// PROCESS
        copy_to_buffer(&replyBuffer, "DEG\r\n", 5, true);
        comms_send(&replyBuffer);
    } else if (compare_strings(packet->buffer, commands[COMMAND_SPD_INDEX], COMMAND_LENGTH)) {
        packet->tail -= 3;
        
        int16_t speed = get_param(packet, ',');
        if (speed == INVALID_PARAM) {
            return; // Invalid packet
        }
        packet->tail--; // move tail to end of next parameter
        // Get the first parameter
        int16_t jointNum = get_param(packet, commands[COMMAND_SPD_INDEX][COMMAND_LENGTH - 1]);
        if (jointNum == INVALID_PARAM) {
            return; // Invalid packet
        }
